...
### ✅ 免连线模式:本地配置文件(v3.6.0 新增,v3.6.1 修复「配置」改为可选输入)

- **背景**:节点一多,每个生成/编辑节点都去连「配置」节点显繁琐。v3.6.0 起,生成/编辑节点在**未连「配置」输入**时,自动回退读取插件根目录的 `local_config.json`(`.gitignore` 已忽略、不进版本库):
  > **v3.6.1 修复**:v3.6.0 的「配置」声明在 `required` 段,ComfyUI 前端对未连线输入会直接拒绝执行(`Required input is missing`),文件回退逻辑根本走不到。v3.6.1 起「配置」改为**可选输入**——端口仍在、可连可不连,免连线模式才真正可用。

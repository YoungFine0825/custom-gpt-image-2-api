...
- **免连线模式走本地文件**(v3.6.0 新增):生成/编辑节点**未连「配置」输入**时,自动读取插件根目录的 `local_config.json`(`.gitignore` 已忽略、不进版本库),配一次全插件生效。优先级:显式连线 > 配置文件。凭据只在本机磁盘,不进工作流、不经过浏览器 localStorage,比配置节点暴露面更小;校验与配置节点共用同一入口,行为一致。
